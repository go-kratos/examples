...
// The DictDetailFunc type is an adapter to allow the use of ordinary
// function as DictDetail mutator.
type DictDetailFunc func(context.Context, *ent.DictDetailMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DictDetailFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DictDetailMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DictDetailMutation", m)
}
